...
namespace RedBjorn.SuperTiles.Editors.HealthRule.Submenus
{
    public class Create : ISubmenu
    {
        ...
        public Create(HealthRuleWindow window)
        {
            window.UpdatePopUp();
        }
        ...
        public void Draw(HealthRuleWindow window)
        {
            ...
            window.ConditionIndex = EditorGUILayout.Popup("Condition", window.ConditionIndex, window.Conditions);
            window.ConverterIndex = EditorGUILayout.Popup("Converter", window.ConverterIndex, window.Converters);

            if (GUILayout.Button("Create"))
            {
                window.RuleCreateNew(window.Conditions[window.ConditionIndex], window.Converters[window.ConverterIndex]);
